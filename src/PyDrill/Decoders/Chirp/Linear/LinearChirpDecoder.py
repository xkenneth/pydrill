...
from PyDrill.Objects import Chirp,Pulse,Frame,SubFrame
from copy import deepcopy
from Ft.Xml import Parse
import mx


class LinearChirpDecoder(Decoder.Decoder):
	...
	def decode(self,pulses,trash,jitter,debug=False):
		...
		for chirp in self.chirps:
			if len(chirp)==4:
				...
				try:
					temp,exists = pulses.getPulsesBefore(p,debug=False)
				except Exception, e:
					print "!",e
					exists = False
					pass
		...
		for chirp in self.chirps:
			...
			#take the timeStamp of the chirp to be that of the first pulse
			saveTimeStamp = pulses[0].timeStamp
				...
				if lastAverage:
					if len(chirp.deltas)==len(lastChirp.deltas):
						if average<lastAverage:
							...
							lastChirp = chirp
							lastChirp.timeStamp = saveTimeStamp
...
if __name__ == '__main__':
	import unittest
	class LinearChirpDecoderTests(unittest.TestCase):
		def setup(self):
			...
			cd = LinearChirpDecoder() #get a chirp decode instance and populate it
			cd.addChirps(file='chirps.xml')
			self.cd = cd

		def loadXml(self):
        
			xmlFile = file('chirps.xml','r') #load the chirps
			doc = Parse(xmlFile)
			...
			for subFrameTag in subFrameTags:
				self.subFrames.append(SubFrame.SublsFrame(xml=subFrameTag))
